...
import {
  ...
  revealNextClue, registerCorrectGuess, registerWrongGuess,
...
export function renderQuienSoy(container, params) {
  const { catId } = params;
  ...
  state = createGameState('quiensoy', catId, selected);
...
function renderRound(container) {
  ...
  // Build clues for this player and store in modeConfig
  state.modeConfig.clues = buildQuienSoyClues(player);
  ...
  container.innerHTML = `
    <div class="header">
      ...
      <h1>${catName}</h1>
      <div class="header-sub">Quién Soy</div>
    </div>
    <div class="game">
      ...
      <div class="game-mode-label">¿Quién soy? Adiviná con las pistas</div>
...
function renderClues() {
  ...
  if (!cluesArea) return;
  const clues = state.modeConfig.clues || [];
    ...
    if (i < state.cluesRevealed) {
      ...
      html += `
        <div class="clue-card">
          ...
          <span class="clue-flag">${c.emoji}</span>
...
function renderGuessArea() {
  ...
  guessArea.innerHTML = `
    <div class="guess-row">
      <input type="text" class="guess-input" id="guess-input" placeholder="¿Quién soy?" autocomplete="off" autocorrect="off" spellcheck="false">
  ...
  skipBtn.addEventListener('click', () => { registerGiveUp(state); showAnswer(false); });
}
...
function showAnswer(correct) {
  ...
  // Reveal all clues
  state.cluesRevealed = (state.modeConfig.clues || []).length;
  renderClues();
