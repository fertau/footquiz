...
export function renderPasaporte(container, params) {
  const { catId } = params;
  ...
  state = createGameState('pasaporte', catId, selected);
...
function renderRound(container) {
  ...
  container.innerHTML = `
    <div class="header">
      ...
      <h1>${catName}</h1>
      <div class="header-sub">El Pasaporte</div>
    </div>
    <div class="game">
      ...
      <div style="text-align:center;color:var(--gray-600);font-size:0.9rem;margin:8px 0;">¿En qué países jugó? Adiviná quién es</div>
...
function renderClues() {
  ...
  let html = '';
  for (let i = 0; i < player.paises.length; i++) {
    if (i < state.cluesRevealed) {
      const pais = player.paises[i];
      const isNacionalidad = i === 0 && pais === player.nacionalidad;
      html += `
        <div class="clue-card">
          <span class="clue-number">${i + 1}</span>
          <span class="clue-flag" style="font-size:2.2rem;">${getFlag(pais)}</span>
          <span class="clue-text">${isNacionalidad ? 'Nació acá' : 'Jugó acá'}</span>
        </div>
      `;
    }
  }
...
function showAnswer(correct) {
  ...
  state.cluesRevealed = player.paises.length;
  renderClues();
